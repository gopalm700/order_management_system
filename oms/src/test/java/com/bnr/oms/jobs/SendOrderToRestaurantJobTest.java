...
public class SendOrderToRestaurantJobTest {
  ...
  private SendOrderToResturantJob sendOrderToResturantJob;

  @Before
  public void setUp() {
    reset(repository, orchestrationService);
    sendOrderToResturantJob = new SendOrderToResturantJob(repository, orchestrationService);
  }
  ...
  public void shouldReturnWhenThereIsNoOrder() throws InterruptedException {
    ...
    sendOrderToResturantJob.notifyEligibleOrders();
  ...
  public void shouldNotifyIfOrderExists() throws InterruptedException {
    ...
    doNothing().when(orchestrationService).orchestrate(any(OrderNotifyEvent.class));
    when(repository.saveAll(any(Iterable.class))).thenReturn(new ArrayList<>());
    sendOrderToResturantJob.notifyEligibleOrders();
    verify(repository, times(1))
        .findAllByStatusAndNotifyTimeBeforeOrderByNotifyTimeAsc(any(OrderStatus.class),
            any(Date.class));
    verify(orchestrationService, times(1)).orchestrate(any(OrderNotifyEvent.class));
  ...
  public void shouldRetryWhenFails() throws InterruptedException {
    ...
    verify(repository, times(3))
        .findAllByStatusAndNotifyTimeBeforeOrderByNotifyTimeAsc(any(OrderStatus.class),
            any(Date.class));
    verify(orchestrationService, times(3)).orchestrate(any(OrderNotifyEvent.class));
